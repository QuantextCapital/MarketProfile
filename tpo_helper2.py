...
def get_dayrank(dist_df, mean_val):
    """
    Calculates the day rank based on the distribution data frame `dist_df` and a `mean_val`
     dictionary containing the mean values for 'rf_mean' and 'volume_mean'.
    Assigns day types to each row in the data frame based on TPO distribution and calculates
     numerical values for each day type for calculating total strength at the end.
    Calculates various factors including 'vold_zscore' and 'rfd_zscore' based on mean values and
    assigns 'Volume_Factor' and 'Rotation_Factor' accordingly.
    Generates a 'power' value based on several factors and returns the ranked data frame along
     with a breakdown of individual factors.
    """
    ...
    ranking_df['Volume_Factor'] = np.where(ranking_df['Volume_Factor'].isnull(), 0, ranking_df['Volume_Factor'])
    ...
    ranking_df['daytype_num'] = np.where(ranking_df['daytype_num'].isnull(), 0, ranking_df['daytype_num'])
...
def get_ibrank(mean_val, ranking):
    ...
    if ibrankdf.ibvol.isnull().values.any():
        ibrankdf['ibvol_zscore'] = (abs(ibrankdf.ibvol) - vol_mean) / abs(ibrankdf.ibvol).std(
            ddof=0)
    else:
        ibrankdf['ibvol_zscore'] = 0
