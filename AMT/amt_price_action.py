...
# path = r"G:\anaconda\Scripts\data\nifty_bank\minute\SBIN.csv"
#path = r"G:\anaconda\Scripts\data\nifty_bank\minute\SBIN.csv"
...
symbol = "NIFTY50-Minute"
df = pd.read_pickle(f'C:/Users/alex1/OneDrive/Public/{symbol}.pkl')
split = int(len(df) / 2)
...
long_condition = ("(merged_df2['Close'] > merged_df2['VAL'].shift(1)) & "
                "(merged_df2['Close'].shift(1) < merged_df2['VAL'].shift(2)) & "
                 #  "(merged_df2['SMA_small'] > merged_df2['SMA_long']) &"
                 #  "(merged_df2['POC_small'] > merged_df2['POC_long']) &"
                 "(merged_df2['rf'] > merged_df2['rf'].shift(1)) &"
                  # " (merged_df2['TPO_Net'] > 100) & "
                 "(merged_df2['POC_trend'] == 1) & "
                  "(merged_df2['DayOpen'] < merged_df2['VAH']) &"
                 "(merged_df2['time'] >= 94400) & (merged_df2['time'] <= 132900)")

short_condition = ("(merged_df2['Close'] < merged_df2['VAH'].shift(1)) &"
                  "(merged_df2['Close'].shift(1) > merged_df2['VAH'].shift(2)) &"
                  #  "(merged_df2['SMA_small'] < merged_df2['SMA_long']) &"
                  #  "(merged_df2['POC_small'] < merged_df2['POC_long']) &"
                  "(merged_df2['rf'] < merged_df2['rf'].shift(1)) &"
                  # "(merged_df2['TPO_Net'] > 100) &"
                  "(merged_df2['POC_trend'] == -1) &"
                 "(merged_df2['DayOpen'] > merged_df2['VAL']) &"                   
                  "(merged_df2['time'] >= 94400) & (merged_df2['time'] <= 132900)")
        ...
        if not self.position:
            ...
            if self.position.is_long:
                if (self.data.time == 152900
                        ...
                        # or (self.data.Close[-1] > self.entry_price_buy + (self.data.AR[-1]/self.n2))
            ...
            if self.position.is_short:
                if (self.data.time == 152900
                        ...
                        # or (self.data.Close[-1] < self.entry_price_sell - (self.data.AR[-1]/self.n2))
                ):
                    self.position.close()

backtest = Backtest(merged_df2, EightyPercent, cash=500_000,
...
# C optimize
opt_stats,heatmap = backtest.optimize(n1=range(1, 15),
                              # n2=range(1, 7),
                    maximize='Return [%]',
                              method='grid',
                    constraint=lambda x: x.n1 >= x.n1,
                              return_heatmap=True,
                              random_state=2)
print(opt_stats)
best_n1 = opt_stats._strategy.n1
print(f"The best n1 value is: {best_n1}")
# best_n2 = opt_stats._strategy.n2
# print(f"The best n1 value is: {best_n2}")

print(heatmap)

ec = stats['_equity_curve']
eqfig = equity_curve_plot(ec,merged_df2.copy())
plot(eqfig)
...
with open(f'backtestreport_{symbol}_final.txt', 'w') as f:
    f.write(str(opt_stats) + '\n' + btreport[0] + '\n' + f'SL=average range 20/2 and no profit target\n Long Condition:'
                                                     f' {long_condition}+\nShort '
                                                     f'Condition'
                                                     f':{short_condition}')
